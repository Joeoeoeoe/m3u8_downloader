...
实现了一个图形化的 m3u8 下载器。大多数“非官方”“非主流”的视频网站都是用 m3u8 格式进行分片播放，本项目主要针对这些网站进行视频下载。支持直接输入 m3u8 地址下载，支持输入网址嗅探 m3u8 资源并下载，支持批量下载。下载过程中分片并保留下载日志。
...
- **两种下载方式**：既能粘贴 m3u8 直链下载，也能粘贴网页地址自动识别视频资源。
- **批量任务**：一条输入可展开成多个下载任务，适合连载、合集、分页内容。
- **复杂页面**：支持递归识别、多轮识别、页面交互（可配置），提高资源发现成功率。
- **并发可调节**：可根据电脑和网络情况调整下载并发，平衡速度与稳定性。
- **日志**：可实时看到识别、下载、重试、合成状态，排查问题更直观。
- **多输出格式**：支持常见视频/音频输出格式，便于后续播放与整理。
