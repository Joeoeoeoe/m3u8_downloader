...
- `m3u8` 监测：通过网络请求/响应监听与页面内容提取持续发现资源
- action 执行：按规则触发页面交互（如 `click`、`wait_for_selector`、`scroll`）
- 两者职责不同且同时存在：action 负责“触发页面变化”，监测负责“捕获变化产生的资源”
- action 顶层始终串行；`m3u8` 监测不会等待 action 全部结束后才开始

### 3.4 串行流程的时序语义

- 后续 action 的条件检查按串行顺序发生，前一个 action 结束后才会开始下一个
- `wait_for_selector` 判断的是“当前状态”，每次轮询都会重新检查，不依赖“曾经出现过”的历史事件
- 如果条件在 action 开始时已经满足，会立即返回
- **如果条件短暂出现后又消失，等该 action 真正开始时可能已不满足，因此可能超时**
- 示例：`wait_for_selector(A)` -> `wait(1000)` -> `wait_for_selector(B)`
- 在该示例中，`B` 的检查一定晚于 `A` 结束；但网络层 `m3u8` 监听在整个流程中持续运行
